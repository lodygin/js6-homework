...
function checkYear(str) {
  return (/^[1-2]([09][0-9][0-9]|100)$/).test(str);
}
...
function createLinksFromDomains(str) {
  ...
  let dommenArray = str.match(/(http:\/\/|https:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}/g);

  for (let i = 0; i < dommenArray.length; i++) {
    str = str.replace(dommenArray[i], `<a href="${dommenArray[i]}">${dommenArray[i].replace(/(http:\/\/|https:\/\/)/, '')}</a>`);
  }
